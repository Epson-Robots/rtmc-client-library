...
{
  cout<<"Starting epson_rtmc_demo"<<endl;
  string password;
  string rb_model;
  ...
  for(int i=0;i<=(DEMO_PERIOD+buff_size);i++)
  {
    ...
    catch(short err)
    {
        ...
        )
      {
        ...
        }while(rc_status.err_num>0 || rc_status.eStop==RC_ESTOP_ON || rc_status.safeGuard == RC_SG_OPEN);
  ...
  cout<<"epson_rtmc_demo has been completed"<<endl;
  return 0;
