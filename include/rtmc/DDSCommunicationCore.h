//Original Copyright 2016 Proyectos y Sistemas de Mantenimiento SL (eProsima).
//Modifications Copyright 2025 Seiko Epson Corporation
//This file is a modified version of DDSCommunication.h, which was originally generated by fastddsgen.
//The RRDStruct class has been placed within a namespace
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

/*!
 * @file DDSCommunication.h
 * This header file contains the declaration of the described types in the IDL file.
 *
 * This file was generated by the tool fastddsgen.
 */

#ifndef _FAST_DDS_GENERATED_DDSCOMMUNICATION_H_
#define _FAST_DDS_GENERATED_DDSCOMMUNICATION_H_

#include <array>
#include <bitset>
#include <cstdint>
#include <map>
#include <string>
#include <vector>

#include <fastcdr/cdr/fixed_size_string.hpp>
#include <fastcdr/xcdr/external.hpp>
#include <fastcdr/xcdr/optional.hpp>



#if defined(_WIN32)
#if defined(EPROSIMA_USER_DLL_EXPORT)
#define eProsima_user_DllExport __declspec( dllexport )
#else
#define eProsima_user_DllExport
#endif  // EPROSIMA_USER_DLL_EXPORT
#else
#define eProsima_user_DllExport
#endif  // _WIN32

#if defined(_WIN32)
#if defined(EPROSIMA_USER_DLL_EXPORT)
#if defined(DDSCOMMUNICATION_SOURCE)
#define DDSCOMMUNICATION_DllAPI __declspec( dllexport )
#else
#define DDSCOMMUNICATION_DllAPI __declspec( dllimport )
#endif // DDSCOMMUNICATION_SOURCE
#else
#define DDSCOMMUNICATION_DllAPI
#endif  // EPROSIMA_USER_DLL_EXPORT
#else
#define DDSCOMMUNICATION_DllAPI
#endif // _WIN32

namespace eprosima {
namespace fastcdr {
class Cdr;
class CdrSizeCalculator;
} // namespace fastcdr
} // namespace eprosima


namespace epson_rtmc_client
{
        /*!
        * @brief This class represents the structure RRDSturuct defined by the user in the IDL file.
        * @ingroup DDSCommunication
        */
        class RRDSturuct
        {
        public:

        /*!
        * @brief Default constructor.
        */
        eProsima_user_DllExport RRDSturuct();

        /*!
        * @brief Default destructor.
        */
        eProsima_user_DllExport ~RRDSturuct();

        /*!
        * @brief Copy constructor.
        * @param x Reference to the object RRDSturuct that will be copied.
        */
        eProsima_user_DllExport RRDSturuct(
                const RRDSturuct& x);

        /*!
        * @brief Move constructor.
        * @param x Reference to the object RRDSturuct that will be copied.
        */
        eProsima_user_DllExport RRDSturuct(
                RRDSturuct&& x) noexcept;

        /*!
        * @brief Copy assignment.
        * @param x Reference to the object RRDSturuct that will be copied.
        */
        eProsima_user_DllExport RRDSturuct& operator =(
                const RRDSturuct& x);

        /*!
        * @brief Move assignment.
        * @param x Reference to the object RRDSturuct that will be copied.
        */
        eProsima_user_DllExport RRDSturuct& operator =(
                RRDSturuct&& x) noexcept;

        /*!
        * @brief Comparison operator.
        * @param x RRDSturuct object to compare.
        */
        eProsima_user_DllExport bool operator ==(
                const RRDSturuct& x) const;

        /*!
        * @brief Comparison operator.
        * @param x RRDSturuct object to compare.
        */
        eProsima_user_DllExport bool operator !=(
                const RRDSturuct& x) const;

        /*!
        * @brief This function sets a value in member commandID
        * @param _commandID New value for member commandID
        */
        eProsima_user_DllExport void commandID(
                uint16_t _commandID);

        /*!
        * @brief This function returns the value of member commandID
        * @return Value of member commandID
        */
        eProsima_user_DllExport uint16_t commandID() const;

        /*!
        * @brief This function returns a reference to member commandID
        * @return Reference to member commandID
        */
        eProsima_user_DllExport uint16_t& commandID();


        /*!
        * @brief This function sets a value in member dwordParam
        * @param _dwordParam New value for member dwordParam
        */
        eProsima_user_DllExport void dwordParam(
                uint32_t _dwordParam);

        /*!
        * @brief This function returns the value of member dwordParam
        * @return Value of member dwordParam
        */
        eProsima_user_DllExport uint32_t dwordParam() const;

        /*!
        * @brief This function returns a reference to member dwordParam
        * @return Reference to member dwordParam
        */
        eProsima_user_DllExport uint32_t& dwordParam();


        /*!
        * @brief This function copies the value in member exData
        * @param _exData New value to be copied in member exData
        */
        eProsima_user_DllExport void exData(
                const std::vector<uint8_t>& _exData);

        /*!
        * @brief This function moves the value in member exData
        * @param _exData New value to be moved in member exData
        */
        eProsima_user_DllExport void exData(
                std::vector<uint8_t>&& _exData);

        /*!
        * @brief This function returns a constant reference to member exData
        * @return Constant reference to member exData
        */
        eProsima_user_DllExport const std::vector<uint8_t>& exData() const;

        /*!
        * @brief This function returns a reference to member exData
        * @return Reference to member exData
        */
        eProsima_user_DllExport std::vector<uint8_t>& exData();

        private:

        uint16_t m_commandID{0};
        uint32_t m_dwordParam{0};
        std::vector<uint8_t> m_exData;

        };
}

#endif // _FAST_DDS_GENERATED_DDSCOMMUNICATION_H_



